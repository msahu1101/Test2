...
@RequestMapping("/v3")
@Log4j2
public class TokenV3Controller extends BaseController {
    ...
    public void generateToken(HttpServletRequest request, @RequestHeader Map<String, String> headers, @CookieValue(
        ...
        // sessionAge can be passed as input header if gen 3 clients needs to
        // reduce the
        // idle timeout to < 1h. Used in automated testing
        if (headers.containsKey(ServiceConstant.SESSION_AGE)
                && Integer.parseInt(headers.get(ServiceConstant.SESSION_AGE)) < 3600) {
            request.getSession().setMaxInactiveInterval(Integer.parseInt(headers.get(ServiceConstant.SESSION_AGE)));
        }
